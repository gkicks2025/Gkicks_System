# Simple Interactive VPS Deployment Script for GKicks
param(
...
# Get username
$USER = Read-Host "Enter your VPS username"
...
scp "deploy-gkicks-complete.sh" "$USER@$VPS_IP":/tmp/gkicks-deployment/deploy-gkicks-complete.sh
...
ssh "$USER@$VPS_IP" "chmod +x /tmp/gkicks-deployment/deploy-gkicks-complete.sh"
...
ssh "$USER@$VPS_IP" "cd /tmp/gkicks-deployment; DOMAIN=$DOMAIN ./deploy-gkicks-complete.sh"
