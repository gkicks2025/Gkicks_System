...
export default function UnisexPage() {
  ...
  useEffect(() => {
    async function fetchUnisexProducts() {
      try {
        const response = await fetch('/api/products?category=unisex')
        if (!response.ok) {
          throw new Error('Failed to fetch unisex products')
        }
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="mb-6 sm:mb-8">
            ...
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 max-w-2xl">
              Versatile footwear designed for everyone. Classic styles and modern designs that transcend gender boundaries, perfect for any wardrobe.
            </p>
          </div>

          {/* Product Grid */}
          <ProductGrid category="unisex" products={products} onWishlistUpdate={handleWishlistUpdate} />
        </div>
      </main>
    </div>
  )
